...
const plans = [
  ...
  },
  {
    ...
    missing: [], cta: "Попробовать 14 дней", popular: true,
  },
  {
    ...
    missing: [], cta: "Связаться с нами", popular: false,
  },
];
    ...
    <div className="min-h-screen bg-[#080612]">
      ...
      <section className="gradient-hero pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10 text-center">
          <Badge className="mb-6 gradient-primary text-white border-0 px-4 py-1.5">Цены</Badge>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <span className={`text-sm font-medium ${yearly ? "text-white" : "text-white/40"}`}>
              Годовой
              <Badge className="ml-2 bg-green-500/20 text-green-400 border-green-500/30 text-xs">−33%</Badge>
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 -mt-10">
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.map((plan, i) => (
              <div key={plan.name} className={`glass rounded-2xl p-8 relative reveal delay-${i * 150 + 100} ${plan.popular ? "ring-2 ring-purple-500 glow-primary" : ""}`}>
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <Badge className="gradient-primary text-white border-0 px-4 py-1 font-semibold">Популярный</Badge>
                  </div>
                )}
                ...
                <div className="mb-6">
                  {plan.price.monthly === 0 ? (
                    <div className="text-4xl font-black text-white font-['Manrope']">Бесплатно</div>
                  ) : (
                ...
                <Button
                  className={`w-full mb-8 font-semibold ${plan.popular ? "gradient-primary text-white border-0 hover:opacity-90" : "border-white/20 text-white bg-transparent hover:bg-white/10"}`}
            ...
            {faq.map((item, idx) => (
              <div key={idx} className={`glass rounded-2xl overflow-hidden reveal delay-${idx * 100 + 100}`}>
                ...
                >
                  ...
                  <Icon name={openFaq === idx ? "ChevronUp" : "ChevronDown"} size={18} className="text-white/50 flex-shrink-0 ml-4" />
                </button>
                {openFaq === idx && (
